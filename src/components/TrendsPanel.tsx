    ...
    <div className="mt-5" data-testid="yearly-history">
      <div className="mb-2">
        ...
        <p className="mt-0.5 text-xs text-[var(--color-ink-muted)]">
          Last {yearsBack} years archived
          {filtered ? ` · filtered to ${filterLabel}` : ''}
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      {data.yearly.length > 0 ? (
        <YearlyBars
          yearly={data.yearly}
          yearsBack={backfill?.yearsBack ?? 5}
          scaleMax={
            data.yearlyScaleMax ?? Math.max(1, ...data.yearly.map((y) => y.observationCount))
          }
