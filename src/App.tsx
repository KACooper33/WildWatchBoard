...
export default function App() {
  ...
  const allObservations = observationsQuery.data?.observations ?? []

  const filteredObservations = useMemo(
    () => filterObservationsByTaxa(allObservations, appliedTaxa),
    [allObservations, appliedTaxa],
  )

  const displayMetrics = useMemo(() => {
    if (!metricsQuery.data) return undefined
    if (appliedTaxa.length === 0) return metricsQuery.data
    return computeMetricsFromObservations(
      regionId,
      windowDays,
      filteredObservations,
      metricsQuery.data.cachedAt,
    )
  }, [
    metricsQuery.data,
    appliedTaxa,
  ...
  const filterBusy =
    ...
    (appliedTaxa.length > 0 &&
      (observationsQuery.isFetching || trendsQuery.isFetching))
    ...
    <div className="min-h-dvh">
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-4 sm:gap-5 sm:px-6 sm:py-6">
            ...
            {filterBusy ? (
              ...
              <>
                ...
                <span className="text-[var(--color-ink-muted)]">
                  · snapshot, trends, map, and species list
                </span>
              </>
            )}
          </div>
        ...
        <RegionSnapshot
          ...
          groupCounts={metricsQuery.data?.byIconicTaxon}
